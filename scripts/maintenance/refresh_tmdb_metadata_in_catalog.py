"""
MAINTENANCE: Refresh TMDB metadata for already-matched catalog rows.

Purpose:
  Re-fetches TMDB details and credits for catalog_items rows that are
  already matched (tmdb_id set) to update stale metadata like cast,
  genres, posters, or popularity scores.

Tables/fields mutated:
  catalog_items: director, film_released, genres, top_cast, country_of_origin,
                 tmdb_poster_path, tmdb_backdrop_path, tmdb_vote_average,
                 tmdb_vote_count, tmdb_popularity, tmdb_last_refreshed_at

Safe mode: No dry-run (writes directly)
Cron-safe: NO — manual use only. Violates the "match once, never rematch" rule
           for TMDB enrichment. Use only for metadata refresh, not re-matching.
"""
