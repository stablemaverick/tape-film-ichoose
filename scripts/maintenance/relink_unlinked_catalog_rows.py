"""
MAINTENANCE: Relink unlinked catalog rows to films.

Purpose:
  Finds catalog_items rows that have been enriched with TMDB data
  (tmdb_id present) but have no film_id, and attempts to link them
  to existing films or create new film records.

Tables/fields mutated:
  catalog_items: film_id, film_link_status, film_link_method, film_linked_at
  films: may insert new rows

Safe mode: Preview logging before writes
Cron-safe: NO — manual/one-off use only (use build_films_from_catalog.py for operational linking)
"""
