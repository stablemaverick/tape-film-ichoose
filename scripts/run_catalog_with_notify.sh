#!/usr/bin/env bash
# =============================================================================
# Catalog pipeline with Slack start/success/failure notifications. Preserves exit code.
...
PIPELINE="/opt/tape-film-ichoose/pipeline/run_catalog_sync.sh"
...
slack_final_line_for_pipeline() {
  ...
  obs_ec=$?
  set -e
  fmt_py="$(mktemp "${TMPDIR:-/tmp}/slack_fmt_catalog.XXXXXX.py")"
  cat >"${fmt_py}" <<'PY'
import json
import sys
